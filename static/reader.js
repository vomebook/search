...
function updateSearchState(patch) { return readerRuntime.update("search", patch); }
function publishSearchResults(generation, results) { return isReaderGenerationCurrent("search", generation) && updateSearchState({ results }); }
function updatePanelState(patch) { return readerRuntime.update("panel", patch); }
updateDocumentState({ title: String(initialTitle).trim() || sourceName, zoom: Math.min(4, Math.max(0.25, Number(localStorage.getItem("reader-zoom") || 100) / 100)) });
function applyReaderMetadata(data) { if (!data) return; resolvedReaderData = { ...(resolvedReaderData || {}), ...data }; readerRuntime.update("source", { metadata: resolvedReaderData }); applyReaderPathMetadata(resolvedReaderData); const originalTitle = String(resolvedReaderData.title || "").trim(), originalExtension = String(resolvedReaderData.original_extension || (extension === "pdf-pages" ? "pdf" : "")).toLowerCase().replace(/^\./, ""); if (originalTitle) { const resolvedTitle = originalTitle + (originalExtension && !originalTitle.toLowerCase().endsWith(`.${originalExtension}`) ? `.${originalExtension}` : ""); updateDocumentState({ title: resolvedTitle }); document.querySelector("#title").textContent = resolvedTitle; document.title = `${resolvedTitle} - VoiceOfML Reader`; } }
...
for (const [selector, label] of [["#back", "返回"], ["#page-prev", "上一页"], ["#page-next", "下一页"], ["#page-number", "页码"], ["#zoom-out", "缩小"], ["#zoom-in", "放大"], ["#zoom", "缩放百分比"], ["#bookmark-ribbon", "添加书签"], ["#history", "阅读选项"]]) document.querySelector(selector)?.setAttribute("aria-label", label);
...
contentSanitizerObserver = new MutationObserver(() => { const seen = new Set(); for (const article of document.querySelectorAll(".foliate-continuous > article[data-section]")) { article.querySelectorAll("style,link[rel=stylesheet]").forEach((node) => node.remove()); const index = article.dataset.section; if (seen.has(index)) article.remove(); else seen.add(index); } }).observe(content, { childList: true, subtree: true });
...
document.querySelector("#title").textContent = documentState.title + (extension && !documentState.title.toLowerCase().endsWith(`.${extension}`) ? `.${extension}` : ""); readerPath.textContent = readerPathLabel; readerPath.hidden = !readerPathLabel; status.hidden = true; document.title = documentState.title + " - VoiceOfML Reader";
new MutationObserver(() => { updateDocumentState({ title: document.querySelector("#title").textContent }); }).observe(document.querySelector("#title"), { childList: true });
...
function setZoom(percent, persist = true) { const anchor = foliateScrollAnchors.capture(), normalized = VoiceOfMLReader.clampNumber(percent, 25, 400, 100), horizontalCenter = viewport.scrollWidth ? (viewport.scrollLeft + viewport.clientWidth / 2) / viewport.scrollWidth : 0; updateDocumentState({ zoom: normalized / 100 }); content.style.setProperty("--reader-zoom", String(documentState.zoom)); zoomInput.value = String(normalized); if (htmlFrame && htmlFrame.contentDocument) htmlFrame.contentDocument.documentElement.style.zoom = String(documentState.zoom); if (pdfDocument) rerenderVisiblePdfPages(); if (capability.mode === "foliate") content.style.setProperty("--reader-zoom", String(documentState.zoom)); viewport.scrollLeft = horizontalCenter * viewport.scrollWidth - viewport.clientWidth / 2; localStorage.setItem("reader-zoom", String(normalized)); foliateScrollAnchors.restore(anchor); if (persist) scheduleSave(); }
const handlePageNavigationFailure = () => fail("原文件加载失败，请检查网络后重试，或下载原文件。", "READER_NAVIGATION");
 setZoom(documentState.zoom * 100, false); for (const [id, delta] of [["#zoom-out", -10], ["#zoom-in", 10]]) document.querySelector(id).addEventListener("click", () => setZoom(Number(zoomInput.value) + delta));
...
async function goToPage(value) { if (!documentState.pageCount) return; const page = VoiceOfMLReader.clampNumber(value, 1, documentState.pageCount, 1); await pdfShellsReady; const shell = content.querySelector(`.reader-page[data-page="${page}"], .reader-docx-page[data-page="${page}"]`); if (shell) { if (capability.mode === "pdf-pages") await renderPdfManifestShell(shell, false, true); else if (shell.classList.contains("reader-page")) await renderPdfShell(shell, false, true); viewport.scrollTop = shell.offsetTop; if (!restorationApplied && documentState.restoredEntry && page === documentState.restoredEntry.page && documentState.restoredEntry.pageOffset) { viewport.scrollTop += documentState.restoredEntry.pageOffset; restorationApplied = true; } } pageNavigationLockUntil = performance.now() + 500; updateDocumentState({ page }); pageInput.value = String(page); updateTocCurrentMark(); updateProgressTools(); scheduleSave(); }
...
async function navigateTocEntry(index) { if (index < 0 || index >= navigationState.tocEntries.length) return; const generation = nextReaderGeneration("navigation"), entry = navigationState.tocEntries[index]; updateNavigationState({ currentChapterIndex: index }); try { if (foliateContinuous) await activateFoliateTocEntry(entry, generation); else { await entry.activate(); if (isReaderGenerationCurrent("navigation", generation)) setReaderPanelOpen(false, true); } } catch (error) { if (isReaderGenerationCurrent("navigation", generation)) console.warn("Reader navigation failed", error); } }
function setToc(entries) { const normalizedEntries = (entries || []).map((entry) => ({ ...entry, href: String(entry.href || ""), label: cleanTocLabel(entry.label) })); updateNavigationState({ tocEntries: normalizedEntries, currentChapterIndex: normalizedEntries.length ? 0 : -1 }); const tocTab = document.querySelector("#toc-tab"); tocTab.hidden = false; tocTab.textContent = normalizedEntries.length ? "目录" : "阅读"; const previous = document.querySelector("#toc-list"), list = previous.cloneNode(false); previous.replaceWith(list); for (const [index, entry] of normalizedEntries.entries()) { const row = document.createElement("div"); row.className = "panel-item toc-item"; row.style.setProperty("--toc-depth", String(entry.depth || 0)); const link = document.createElement("div"); link.className = "panel-item-main"; link.tabIndex = 0; link.setAttribute("role", "link"); link.textContent = entry.label; const activate = () => { if (!getSelection().toString()) navigateTocEntry(index); }; link.addEventListener("click", activate); link.addEventListener("keydown", (event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); activate(); } }); row.appendChild(link); list.appendChild(row); } updateTocCurrentMark(); }
function cleanTocLabel(value) { return String(value || "未命名章节").replace(/[\uFFFD\u2610-\u2612\u25a1\u0000-\u001F\u007F]/gu, "").replace(/\s{2,}/gu, " ").trim() || "未命名章节"; }
function updateTocCurrentMark() { const rows = [...document.querySelectorAll("#toc-list .toc-item")]; if (documentState.pageCount > 1 && documentState.page > 0) { let pageIndex = -1; for (const [index, row] of rows.entries()) { const match = row.textContent.match(/第\s*(\d+)\s*页/u); if (match && Number(match[1]) <= documentState.page) pageIndex = index; } if (pageIndex >= 0) updateNavigationState({ currentChapterIndex: pageIndex }); } for (const [index, row] of rows.entries()) { let mark = row.querySelector(".toc-current-mark"); if (!mark) { mark = document.createElement("span"); mark.className = "toc-current-mark"; mark.textContent = "✓"; row.appendChild(mark); } mark.hidden = index !== navigationState.currentChapterIndex; row.classList.toggle("is-current", index === navigationState.currentChapterIndex); } }
function syncFoliateScrollLocation() { if (!foliateContinuous || !navigationState.tocEntries.length) return; const rows = [...document.querySelectorAll(".foliate-continuous article[data-section]")], top = viewport.getBoundingClientRect().top + 80; let sectionIndex = -1, sectionNode = null; for (const row of rows) if (row.getBoundingClientRect().top <= top) { sectionIndex = Number(row.dataset.section); sectionNode = row; } if (!sectionNode) return; let best = -1; for (const [index, entry] of navigationState.tocEntries.entries()) { if (entry.sectionIndex !== sectionIndex) continue; let position = sectionNode.getBoundingClientRect().top; if (entry.fragment) { const anchor = foliateSectionQuery(sectionNode, `[id="${CSS.escape(entry.fragment)}"], [name="${CSS.escape(entry.fragment)}"]`); if (anchor) position = anchor.getBoundingClientRect().top; } if (position <= top || best < 0) best = index; } if (best >= 0 && best !== navigationState.currentChapterIndex) { updateNavigationState({ currentChapterIndex: best }); updateTocCurrentMark(); updateProgressTools(); } }
function refreshFoliateWindow() { foliateWindowFrame = 0; if (!foliateChapterRepository) return; const viewportRect = viewport.getBoundingClientRect(); for (const placeholder of document.querySelectorAll(".foliate-continuous article.foliate-section-placeholder[data-section]")) { const rect = placeholder.getBoundingClientRect(); if (rect.bottom > viewportRect.top - 1000 && rect.top < viewportRect.bottom + 1000) foliateChapterRepository.load(Number(placeholder.dataset.section)).catch(() => {}); } }
function scheduleFoliateWindowRefresh() { if (!foliateWindowFrame) foliateWindowFrame = requestAnimationFrame(refreshFoliateWindow); }
function trimFoliateSections() { if (!foliateContinuous || !foliateSectionVirtualizer) return; const marker = viewport.getBoundingClientRect().top + 80, rows = [...document.querySelectorAll(".foliate-continuous article[data-section]")], current = rows.find((row) => { const rect = row.getBoundingClientRect(); return rect.top <= marker && rect.bottom > marker; }) || rows.find((row) => row.getBoundingClientRect().top > marker); if (current) foliateSectionVirtualizer.trim(Number(current.dataset.section)); }
function scheduleFoliateScrollSync() { if (foliateScrollFrame) return; foliateScrollFrame = requestAnimationFrame(() => { foliateScrollFrame = 0; syncFoliateScrollLocation(); foliateScrollAnchors.whenIdle(trimFoliateSections); }); }
async function activateFoliateTocEntry(entry, generation = nextReaderGeneration("navigation")) { const href = String(entry.href || ""), target = await epubRendition?.book?.resolveHref(href), section = target && epubRendition.book.sections[target.index], sections = epubRendition.book.sections.filter((item) => item.linear !== "no"), visibleIndex = section ? sections.indexOf(section) : -1; if (!section || visibleIndex < 0) return; let node = document.querySelector(`.foliate-continuous > article[data-section="${visibleIndex}"]:not(.foliate-section-placeholder)`); if (!node && foliateSectionLoader) node = await foliateSectionLoader(visibleIndex); if (foliateSectionSettler) await foliateSectionSettler(); if (!node || !isReaderGenerationCurrent("navigation", generation)) return; const root = foliateSectionRoot(node), anchorDocument = { getElementById: (id) => root.querySelector(`[id="${CSS.escape(id)}"]`), querySelector: (selector) => root.querySelector(selector) }; let anchor = null; try { anchor = typeof target.anchor === "function" ? target.anchor(anchorDocument) : null; } catch (_) {} if (!anchor) { const fragment = href.split("#")[1] || "", id = fragment ? decodeURIComponent(fragment) : ""; anchor = id ? root.querySelector(`[id="${CSS.escape(id)}"], [name="${CSS.escape(id)}"]`) : null; } if (!isReaderGenerationCurrent("navigation", generation)) return; const destination = anchor || node; foliateScrollAnchors.invalidate(); viewport.scrollTop = Math.max(0, viewport.scrollTop + destination.getBoundingClientRect().top - viewport.getBoundingClientRect().top - 8); updateNavigationState({ currentChapterIndex: navigationState.tocEntries.indexOf(entry) }); updateTocCurrentMark(); setReaderPanelOpen(false, true); }
...
function readerProgressPercent() { if (mediaElement) return mediaElement.duration ? Math.round(mediaElement.currentTime / mediaElement.duration * 1000) / 10 : 0; if (foliateContinuous) { const position = captureFoliateBookmarkPosition(), count = epubBook?.sections?.filter((section) => section.linear !== "no").length || 0, article = position && document.querySelector(`.foliate-continuous article[data-section="${position.foliateSection}"]`), fraction = article ? Math.max(0, Math.min(1, position.foliateOffset / Math.max(1, article.getBoundingClientRect().height))) : 0; return count && position ? Math.round((position.foliateSection + fraction) / count * 1000) / 10 : 0; } if (htmlFrame && htmlFrame.contentDocument) { const doc = htmlFrame.contentDocument.documentElement, win = htmlFrame.contentWindow; return Math.round(Math.max(0, Math.min(1, win.scrollY / Math.max(1, doc.scrollHeight - win.innerHeight))) * 1000) / 10; } const position = viewport.scrollTop / Math.max(1, viewport.scrollHeight - viewport.clientHeight); return Math.round(Math.max(0, Math.min(1, position)) * 1000) / 10; }
...
function captureBookmarkSnapshot() {
  ...
  const shell = documentState.pageCount ? pageAtMarker() : null,
    ...
    scrollTop = viewport.scrollTop,
    pagedDocument =
      documentState.pageCount && !["text", "markdown"].includes(capability.mode),
...
document.querySelector("#bookmark-add").addEventListener("click", async () => {
  ...
  if (panelState.editingBookmark) {
    await VoiceOfMLReaderStore.putBookmark({
      ...panelState.editingBookmark,
      label,
      excerpt,
    });
  } else {
...
bookmarksAllButton.addEventListener("click", () => { updatePanelState({ showingAllBookmarks: !panelState.showingAllBookmarks }); bookmarksAllButton.textContent = panelState.showingAllBookmarks ? "本书书签" : "全部书签"; bookmarksAllButton.setAttribute("aria-pressed", String(panelState.showingAllBookmarks)); renderBookmarks(); });
...
function disposeReader() { if (readerLifecycle.disposed) return; contentSanitizerObserver?.disconnect(); contentSanitizerObserver = null; updateDocumentState({ restorationReady: false }); nextReaderGeneration("navigation"); nextReaderGeneration("search"); nextReaderGeneration("bookmarks"); nextReaderGeneration("pdf"); formatAdapters.dispose(); foliateSectionVirtualizer?.dispose(); foliateChapterRepository?.dispose(); foliateSectionObserver?.disconnect(); loadingObserver.disconnect(); clearTimeout(saveTimer); clearTimeout(themeAnimationTimer); clearTimeout(panelAnimationTimer); if (markerFrame) cancelAnimationFrame(markerFrame); if (foliateScrollFrame) cancelAnimationFrame(foliateScrollFrame); if (foliateWindowFrame) cancelAnimationFrame(foliateWindowFrame); if (epubSeekFrame) cancelAnimationFrame(epubSeekFrame); saveTimer = 0; themeAnimationTimer = 0; panelAnimationTimer = 0; markerFrame = 0; foliateScrollFrame = 0; foliateWindowFrame = 0; epubSeekFrame = 0; try { const destroying = pdfDocument?.destroy?.(); if (destroying?.catch) destroying.catch(() => {}); } catch (_) {} pdfDocument = null; if (mediaElement) { mediaElement.pause(); mediaElement.removeAttribute("src"); mediaElement.load(); mediaElement = null; } foliateSectionVirtualizer = null; foliateChapterRepository = null; foliateSectionLoader = null; foliateSectionSettler = null; epubRendition = null; epubBook = null; chapterManifestObserver?.disconnect(); chapterManifestObserver = null; htmlFrame = null; VoiceOfMLReaderStore.dispose?.(); readerRuntime.dispose(); }
...
function fetchReaderResponse() { const early = window.__VOICE_PDF_PRELOAD__; if (early?.manifest && early.manifestUrl === new URL(sourceUrl, location.href).href) { const pending = early.manifest; early.manifest = null; return pending.then((response) => response?.ok ? response : fetchWithReaderTimeout(sourceUrl, READER_PROXY_TIMEOUT_MS), () => fetchWithReaderTimeout(sourceUrl, READER_PROXY_TIMEOUT_MS)); } if (String(sourceUrl).includes("/api/reader-bucket-resource?")) return fetchWithReaderTimeout(sourceUrl, READER_PROXY_TIMEOUT_MS); return fetchWithReaderTimeout(contentUrl, READER_PROXY_TIMEOUT_MS).then((response) => response.ok ? response : fetchWithReaderTimeout(sourceUrl, READER_PROXY_TIMEOUT_MS), () => fetchWithReaderTimeout(sourceUrl, READER_PROXY_TIMEOUT_MS)); }
...
function preloadPdfFirstPage() {
  try {
    ...
    else if (!navigator.connection?.saveData) { pdfFirstPagePreload = new Image(); pdfFirstPagePreload.decoding = "async"; pdfFirstPagePreload.fetchPriority = "low"; pdfFirstPagePreload.src = pageUrl.href; }
  } catch (_) {}
}
function pdfManifestRootPath() {
  try {
    const url = new URL(sourceUrl, location.href);
    const path = url.pathname === "/api/reader-bucket-resource" ? url.searchParams.get("path") || "" : decodeURIComponent(url.pathname).replace(/^\/datasets\/vomebook\/Reader-Assets\/resolve\/[^/]+\//, "");
    const match = path.match(/^(objects\/[0-9a-f]{2}\/[0-9a-f]{64}\/[0-9a-f]{16})\/page-manifest\.json$/);
    return match?.[1] || "";
  } catch (_) { return ""; }
}
function pdfPageEntry(page) {
  if (!pdfPageManifest || page < 1 || page > pdfPageManifest.pageCount) return null;
  return pdfPageManifest.entries?.[page - 1] || { page, path: `${pdfPageManifest.root}/pages/page-${String(page).padStart(6, "0")}.webp` };
}
function trimPdfManifestImages() { const shells = [...content.querySelectorAll('.reader-page[data-render-state="rendered"]')]; if (shells.length <= 25) return; shells.sort((a, b) => Math.abs(Number(a.dataset.page) - documentState.page) - Math.abs(Number(b.dataset.page) - documentState.page)); for (const shell of shells.slice(25)) { shell.querySelector("img")?.remove(); shell.dataset.renderState = "idle"; } }
async function appendShellsBatched(createShell, total, batchSize) { for (let start = 2; start <= total; start += batchSize) { const fragment = document.createDocumentFragment(); for (let page = start; page < Math.min(start + batchSize, total + 1); page++) fragment.appendChild(createShell(page)); content.appendChild(fragment); await new Promise((resolve) => setTimeout(resolve, 0)); } }
async function renderPdfPages(prepared) {
  ...
  const entries = manifest.version === 1 ? manifest.pages.map((item) => ({ page: Number(item.page), path: String(item.path || "") })).sort((a, b) => a.page - b.page) : null;
  const rootMatch = entries ? entries[0]?.path.match(/^(objects\/[0-9a-f]{2}\/[0-9a-f]{64}(?:\/[0-9a-f]{16})?)\/pages\//) : pdfManifestRootPath()?.match(/^(objects\/[0-9a-f]{2}\/[0-9a-f]{64}\/[0-9a-f]{16})$/);
  const pageCount = entries ? entries.length : manifest.page_count;
  if (!rootMatch || (entries && entries.some((item, index) => item.page !== index + 1 || !new RegExp(`^${rootMatch[1]}/pages/page-[0-9]{6}\\.webp$`).test(item.path)))) throw new Error("PDF_MANIFEST_INVALID");
  pdfPageManifest = { entries, root: rootMatch[1], pageCount }; updateDocumentState({ pageCount }); pageInput.max = String(pageCount); document.querySelector("#page-total").textContent = `/ ${pageCount}`; status.textContent = `${pageCount} 页`;
  ...
  const createShell = (page) => { const shell = document.createElement("section"); shell.className = "reader-page"; shell.dataset.page = String(page); shell.style.aspectRatio = "1 / 1.414"; shell.tabIndex = 0; shell.setAttribute("role", "region"); shell.setAttribute("aria-label", `第 ${page} 页`); if (page === 1 && pdfFirstPagePreload && (!pdfFirstPagePreload.complete || pdfFirstPagePreload.naturalWidth)) shell.appendChild(pdfFirstPagePreload); shell.addEventListener("focus", () => renderPdfManifestShell(shell, false, true)); observer.observe(shell); pageObserver.observe(shell); return shell; };
  const initialPage = VoiceOfMLReader.clampNumber(documentState.restoredEntry?.page, 1, pageCount, 1);
  const firstShell = createShell(1); content.appendChild(firstShell);
  pdfShellsReady = appendShellsBatched(createShell, pageCount, 250);
  if (initialPage !== 1) await pdfShellsReady;
  const targetShell = content.querySelector(`.reader-page[data-page="${initialPage}"]`); await renderPdfManifestShell(targetShell, false, true);
  if (documentState.restoredEntry?.page) await goToPage(initialPage); else syncCurrentPageFromMarker();
}
async function renderPdf(prepared) {
  const pdf = await prepared; pdfDocument = pdf; updateDocumentState({ pageCount: pdf.numPages }); pageInput.max = String(documentState.pageCount); document.querySelector("#page-total").textContent = `/ ${documentState.pageCount}`; status.textContent = `${pdf.numPages} 页`;
...
async function renderPlainText(response) { const pre = document.createElement("pre");
  ...
  } if (asciiPreview.length >= 8) { textNode.appendData(new TextDecoder("utf-8").decode(new Uint8Array(asciiPreview)));
  displayedSampleSize = asciiPreview.length;
  } } else if (displayedSampleSize === sampleSize - value.length) { let asciiLength = 0;
  while (asciiLength < value.length && value[asciiLength] > 0 && value[asciiLength] < 128) asciiLength++;
  if (asciiLength) { textNode.appendData(new TextDecoder("utf-8").decode(value.subarray(0, asciiLength)));
  displayedSampleSize += asciiLength;
  } } } } const sample = new Uint8Array(sampleSize);
  let sampleOffset = 0;
  for (const chunk of chunks) { sample.set(chunk, sampleOffset);
  sampleOffset += chunk.length;
  } const decoder = new TextDecoder(detectTextEncoding(sample, documentState.title));
  let pending = "", frame = 0;
  const flush = () => { frame = 0;
  if (pending) { textNode.appendData(pending);
  pending = "";
  } }, scheduleFlush = () => { if (!frame) frame = requestAnimationFrame(flush);
  };
...
const readingProgressSummary = document.createElement("div"); readingProgressSummary.className = "reader-progress-summary"; readingProgressSummary.innerHTML = '<strong>阅读状态</strong><span class="reader-progress-summary-value">0.0%</span><button class="reader-progress-bookmark text-action" type="button">在当前位置添加书签</button>'; progressTools.prepend(readingProgressSummary); readingProgressSummary.querySelector(".reader-progress-bookmark").addEventListener("click", () => bookmarkRibbon.click()); function updateReadingProgressSummary() { const noToc = !navigationState.tocEntries.length && !mediaElement; readingProgressSummary.hidden = !noToc; if (noToc) readingProgressSummary.querySelector(".reader-progress-summary-value").textContent = `${readerProgressPercent().toFixed(1)}%`; } updateReadingProgressSummary();
function syncTocTab() { const tab = document.querySelector("#toc-tab"); if (!tab) return; tab.hidden = !navigationState.tocEntries.length; if (navigationState.tocEntries.length && navigationState.currentChapterIndex < 0) updateNavigationState({ currentChapterIndex: 0 }); }
syncTocTab();
...
function updateProgressTools() { if (progressSeeking) return; const percent = readerProgressPercent(); progressRange.value = String(percent); progressPercent.textContent = `${percent.toFixed(1)}%`; const hasChapters = navigationState.tocEntries.length > 0, hasPreviousChapter = hasChapters && navigationState.currentChapterIndex > 0, hasNextChapter = hasChapters && navigationState.currentChapterIndex >= 0 && navigationState.currentChapterIndex < navigationState.tocEntries.length - 1; chapterPrev.hidden = !hasChapters; chapterNext.hidden = !hasChapters; chapterPrev.disabled = !hasPreviousChapter; chapterNext.disabled = !hasNextChapter; progressUndo.hidden = !previousProgressState; updateReadingProgressSummary(); placeReadingProgress(); }
...
function flushEpubSeek() { if (epubSeekFrame) { cancelAnimationFrame(epubSeekFrame); epubSeekFrame = 0; epubSeekPromise = seekFoliateProgress(epubSeekTarget); } return epubSeekPromise; }
...
async function runFoliateFullSearch() { const query = fullSearchInput.value.trim(), generation = nextReaderGeneration("search");
  ...
  for await (const group of epubRendition.search({ query })) { if (!isReaderGenerationCurrent("search", generation)) break; let occurrence = 0;
  for (const item of group.subitems || []) { const excerpt = item.excerpt || { pre: "", match: query, post: "" };
  updateSearchState({ results: [...searchState.results, { location: group.label || "电子书位置", cfi: item.cfi, occurrence: occurrence++, snippet: { text: `${excerpt.pre}${excerpt.match}${excerpt.post}`, matchStart: excerpt.pre.length, matchLength: excerpt.match.length, prefix: "", suffix: "" } }] });
  if (searchState.results.length >= 100) break;
  } if (!isReaderGenerationCurrent("search", generation) || searchState.results.length >= 100) break;
  } if (!isReaderGenerationCurrent("search", generation)) return;
  for (const [index, result] of searchState.results.entries()) { const row = document.createElement("button");
  row.type = "button";
  ...
  row.addEventListener("click", () => { updateSearchState({ index });
  navigateFoliateSearchResult(result);
  });
  fullSearchResultsNode.appendChild(row);
  } fullSearchStatus.textContent = searchState.results.length ? `${searchState.results.length}${searchState.results.length >= 100 ? "+" : ""} 个结果` : "没有找到匹配正文";
  }
...
async function navigateFoliateSearchResult(result) { const generation = nextReaderGeneration("navigation"), resolved = await epubRendition?.resolveNavigation?.(result.cfi);
  if (!resolved || !isReaderGenerationCurrent("navigation", generation)) return;
  const section = epubBook?.sections?.[resolved.index], sections = epubBook?.sections?.filter((item) => item.linear !== "no") || [], visibleIndex = section ? sections.indexOf(section) : -1;
  if (visibleIndex < 0 || !foliateSectionLoader) return;
  const node = await foliateSectionLoader(visibleIndex);
  if (foliateSectionSettler) await foliateSectionSettler();
  if (!node || !isReaderGenerationCurrent("navigation", generation)) return;
  clearFullSearchMarks();
  const matches = fullSearchDomMatches(foliateSectionRoot(node), "电子书位置"), match = matches[Math.min(result.occurrence || 0, Math.max(0, matches.length - 1))];
  if (!isReaderGenerationCurrent("navigation", generation) || !match) return;
  const targetRect = match.target.getBoundingClientRect(), viewportRect = viewport.getBoundingClientRect();
  foliateScrollAnchors.invalidate();
  viewport.scrollTop = Math.max(0, viewport.scrollTop + targetRect.top - viewportRect.top - (viewport.clientHeight - targetRect.height) / 2);
  const tocIndex = navigationState.tocEntries.findIndex((entry) => { const target = epubBook.resolveHref(entry.href), targetSection = target && epubBook.sections[target.index];
  return targetSection === section;
  });
  if (tocIndex >= 0) { updateNavigationState({ currentChapterIndex: tocIndex });
  updateTocCurrentMark();
...
async function runFullSearch() { const query = fullSearchInput.value.trim();
  const generation = nextReaderGeneration("search");
  updateSearchState({ query, results: [], index: -1 });
  fullSearchResultsNode.textContent = "";
  fullSearchView.querySelectorAll(".full-search-nav button").forEach((button) => { button.disabled = true;
  });
  if (!query) { fullSearchStatus.textContent = "输入关键词搜索正文";
  return;
  } fullSearchStatus.textContent = "正在搜索正文…";
  try { if (capability.mode === "pdf") publishSearchResults(generation, pdfDocument ? await fullSearchPdfMatches(query) : []);
  else if (["text", "markdown", "docx"].includes(capability.mode)) publishSearchResults(generation, fullSearchDomMatches(content, "阅读位置"));
  else if (capability.mode === "html") publishSearchResults(generation, htmlFrame && htmlFrame.contentDocument && htmlFrame.contentDocument.body ? fullSearchDomMatches(htmlFrame.contentDocument.body, "HTML 阅读位置") : []);
  else throw new Error("此格式没有可搜索文本");
  if (!isReaderGenerationCurrent("search", generation)) return;
  for (const result of searchState.results) { const row = document.createElement("button");
  row.type = "button";
  row.className = "full-search-result";
  const location = document.createElement("small");
  location.className = "full-search-location";
  location.textContent = result.location;
  row.append(location, fullSearchSnippetDom(result.snippet));
  row.addEventListener("click", async () => { await result.activate();
  setReaderPanelOpen(false, true);
  });
  fullSearchResultsNode.appendChild(row);
  } fullSearchStatus.textContent = searchState.results.length ? `${searchState.results.length}${searchState.results.length === 100 ? "+" : ""} 个结果` : "未找到正文匹配";
  } catch (error) { if (isReaderGenerationCurrent("search", generation)) fullSearchStatus.textContent = error.message || "正文搜索不可用";
  } finally { if (isReaderGenerationCurrent("search", generation)) fullSearchView.querySelectorAll(".full-search-nav button").forEach((button) => { button.disabled = !searchState.results.length;
  });
  } }
function moveFullSearch(step) { if (!searchState.results.length) return;
  updateSearchState({ index: (searchState.index + step + searchState.results.length) % searchState.results.length });
  const result = searchState.results[searchState.index];
  Promise.resolve(result.activate()).then(() => fullSearchResultsNode.children[searchState.index].scrollIntoView({ block: "nearest" }));
  }
...
fullSearchButton.addEventListener("click", (event) => { const panel = document.querySelector("#history-panel"); if (panel.classList.contains("is-open") && fullSearchView.hidden === false) { event.preventDefault(); event.stopImmediatePropagation(); clearFullSearchMarks(); selectPanel(navigationState.tocEntries.length ? "toc" : "bookmarks"); } }, true); fullSearchInput.addEventListener("beforeinput", clearFullSearchMarks);
...
async function renderFoliate(prepared) {
  ...
  const settleImages = async (root) => {
    ...
    let timeout = 0;
    const decoded = Promise.allSettled(images.map((image) => typeof image.decode === "function" ? image.decode() : image.complete ? Promise.resolve() : new Promise((resolve) => { image.addEventListener("load", resolve, { once: true }); image.addEventListener("error", resolve, { once: true }); })));
    await Promise.race([decoded, new Promise((resolve) => { timeout = setTimeout(resolve, 2500); })]);
  ...
  const createSection = async (index) => {
      ...
      sectionBody.className = "reader-section-body";
       ...
       sectionBody.innerHTML = body?.innerHTML || "";
        ...
        await settleImages(sectionBody);
       return article;
  };
    ...
    (items) =>
      items
        ...
        .forEach((item) => {
          ...
          if (item.target.classList.contains("foliate-section-placeholder")) { foliateChapterRepository.load(index).then(() => foliateScrollAnchors.whenIdle(trimFoliateSections)).catch(() => {}); return; }
...
async function renderChapterManifest(prepared) {
  const response = await prepared;
  if (!response.ok) throw new Error(`HTTP ${response.status}`);
  const manifest = VoiceOfMLReaderSecurity.validateChapterManifest(JSON.parse(await VoiceOfMLReaderSecurity.readText(response, VoiceOfMLReaderSecurity.LIMITS.manifestBytes)));
...
function searchFormat(mode, query) { fullSearchInput.value = String(query || ""); return mode === "foliate" ? runFoliateFullSearch() : runFullSearch(); }
async function restoreFormat(mode, entry) { if (!entry) return; if (mode === "foliate" && Number.isInteger(entry.foliateSection)) return restoreFoliateBookmarkPosition(entry); if (entry.page) { await goToPage(entry.page); const shell = content.querySelector(`.reader-page[data-page="${entry.page}"], .reader-docx-page[data-page="${entry.page}"]`); if (shell && Number.isFinite(entry.pageOffset)) viewport.scrollTop = shell.offsetTop + entry.pageOffset; return; } restoreProgressState(entry); }
...
function loadPdfDocument() { return import(PDFJS_URL).then((pdfjs) => { pdfjs.GlobalWorkerOptions.workerSrc = PDFJS_WORKER_URL; const options = (url) => ({ url, rangeChunkSize: 1048576, disableAutoFetch: true, disableStream: true, wasmUrl: PDFJS_WASM_URL, cMapUrl: PDFJS_CMAP_URL, cMapPacked: true, standardFontDataUrl: PDFJS_STANDARD_FONT_URL, withCredentials: false }); return loadPdfWithTimeout(pdfjs, options); }); }
...
async function renderPdfManifestShell(shell, force = false, priority = false) { if (!shell || shell.dataset.renderState === "rendering" || (!force && shell.dataset.renderState === "rendered")) return shell?._renderPromise; let finish; shell._renderPromise = new Promise((resolve) => { finish = resolve; }); shell.dataset.renderState = "rendering"; await acquirePdfRenderSlot(priority); try { const entry = pdfPageEntry(Number(shell.dataset.page)); let image = shell.querySelector("img"); if (!image) { image = new Image(); image.alt = `第 ${entry.page} 页`; image.decoding = "async"; shell.appendChild(image); } const target = sourceUrl.includes("/api/reader-bucket-resource?") ? new URL("https://voiceofml-search.hf.space/api/reader-bucket-resource?path=" + encodeURIComponent(entry.path)).href : new URL("/datasets/vomebook/Reader-Assets/resolve/main/" + entry.path, "https://huggingface.co").href; if (image.src !== target) image.src = target; if (!(image.complete && image.naturalWidth)) await new Promise((resolve, reject) => { image.onload = resolve; image.onerror = reject; }); shell.style.aspectRatio = `${image.naturalWidth || 1} / ${image.naturalHeight || 1}`; image.classList.add("ready"); shell.dataset.renderState = "rendered"; shell.dataset.renderUsedAt = String(Date.now()); trimPdfManifestImages(); } catch (error) { shell.querySelector("img")?.remove(); shell.dataset.renderState = "idle"; if (priority) throw error; } finally { scheduleMarkerSync(); releasePdfRenderSlot(); finish(); delete shell._renderPromise; } }
