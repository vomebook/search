...
document.querySelector(".page-controls").hidden = !["pdf", "pdf-pages", "docx"].includes(capability.mode);
document.querySelector(".zoom-controls").hidden = !capability.mode || ["audio", "video"].indexOf(capability.mode) >= 0;
...
function setZoom(percent, persist = true) { const normalized = VoiceOfMLReader.clampNumber(percent, 25, 400, 100), horizontalCenter = viewport.scrollWidth ? (viewport.scrollLeft + viewport.clientWidth / 2) / viewport.scrollWidth : 0; zoom = normalized / 100; content.style.setProperty("--reader-zoom", String(zoom)); zoomInput.value = String(normalized); if (epubRendition && epubRendition.themes) epubRendition.themes.fontSize(`${normalized}%`); if (htmlFrame && htmlFrame.contentDocument) htmlFrame.contentDocument.documentElement.style.zoom = String(zoom); if (pdfDocument) rerenderVisiblePdfPages(); viewport.scrollLeft = horizontalCenter * viewport.scrollWidth - viewport.clientWidth / 2; localStorage.setItem("reader-zoom", String(normalized)); if (persist) scheduleSave(); }
const handlePageNavigationFailure = () => fail("原文件加载失败，请检查网络后重试，或下载原文件。");
...
async function saveProgress() { if (!restorationReady || !validSource(sourceUrl) || historySuppressed) return; syncCurrentPageFromMarker(); const shell = pageCount ? content.querySelector(`.reader-page[data-page="${currentPage}"], .reader-docx-page[data-page="${currentPage}"]`) : null, pageOffset = shell ? Math.max(0, viewport.scrollTop - shell.offsetTop) : 0, htmlScrollTop = htmlFrame && htmlFrame.contentWindow ? htmlFrame.contentWindow.scrollY : 0, readerUrl = new URL(location.href), foliatePosition = captureFoliateBookmarkPosition(); readerUrl.searchParams.delete("return"); readerUrl.searchParams.delete("nav"); const progress = { url: sourceUrl, title, extension, readerUrl: readerUrl.href, page: currentPage, pageCount, pageOffset, epubLocation, mediaTime: mediaElement && Number.isFinite(mediaElement.currentTime) ? mediaElement.currentTime : 0, scrollTop: viewport.scrollTop, htmlScrollTop, zoom: Math.round(zoom * 100), ...(foliatePosition || {}) }, signature = JSON.stringify(progress); if (signature === lastSavedProgress) return progressSaveChain; lastSavedProgress = signature; progressSaveChain = progressSaveChain.catch(() => {}).then(() => VoiceOfMLReaderStore.put({ ...progress, lastReadAt: Date.now() })).catch((error) => { if (lastSavedProgress === signature) lastSavedProgress = ""; console.warn("Reader progress was not saved", error); }); return progressSaveChain; }
function emptyPanel(list, message) { list.innerHTML = `<div class="panel-empty">${message}</div>`; }
function clearSearchHighlights(view) { for (const mark of view.querySelectorAll("mark.search-match")) mark.replaceWith(mark.textContent); view.normalize(); }
...
function setReaderPanelOpen(open, restoreFocus = false) { const panel = document.querySelector("#history-panel"), trigger = document.querySelector("#history"); clearTimeout(panelAnimationTimer); trigger.setAttribute("aria-expanded", String(open)); if (open) { panel.hidden = false; void panel.offsetWidth; panel.classList.add("is-open"); selectPanel(mediaElement ? "media" : "toc"); return; } panel.classList.remove("is-open"); panelAnimationTimer = setTimeout(() => { if (!panel.classList.contains("is-open")) panel.hidden = true; }, 250); if (restoreFocus) trigger.focus({ preventScroll: true }); }
...
function selectPanel(name) { if (name === "media" && mediaTab.hidden) name = "bookmarks"; if (typeof progressTools !== "undefined") progressTools.hidden = !["toc", "media"].includes(name); for (const button of document.querySelectorAll(".reader-panel-tabs button")) { const selected = button.dataset.panel === name; button.setAttribute("aria-selected", String(selected)); button.tabIndex = selected ? 0 : -1; } for (const view of document.querySelectorAll(".reader-panel-view")) view.hidden = view.dataset.panelView !== name; if (name === "bookmarks") renderBookmarks(); if (name === "history") renderHistory(); if (name === "media" && mediaElement) mediaPanel.querySelector(".media-panel-time").textContent = `${formatMediaTime(mediaElement.currentTime)} / ${formatMediaTime(mediaElement.duration)}`; }
async function navigateTocEntry(index) { if (index < 0 || index >= tocEntries.length) return; const generation = ++navigationGeneration, entry = tocEntries[index]; currentChapterIndex = index; try { if (foliateContinuous) await activateFoliateTocEntry(entry, generation); else { await entry.activate(); if (generation === navigationGeneration) setReaderPanelOpen(false, true); } } catch (error) { if (generation === navigationGeneration) console.warn("Reader navigation failed", error); } }
...
async function restoreFoliateBookmarkPosition(entry) { const generation = ++navigationGeneration; if (!foliateSectionLoader || !Number.isInteger(entry.foliateSection)) return; const article = await foliateSectionLoader(entry.foliateSection); if (foliateSectionSettler) await foliateSectionSettler(); if (!article || generation !== navigationGeneration) return; const marker = viewport.getBoundingClientRect().top + 8; viewport.scrollTop = Math.max(0, viewport.scrollTop + article.getBoundingClientRect().top - marker + (Number(entry.foliateOffset) || 0)); if (Number.isInteger(entry.foliateTocIndex) && entry.foliateTocIndex >= 0 && entry.foliateTocIndex < tocEntries.length) currentChapterIndex = entry.foliateTocIndex; updateTocCurrentMark(); setReaderPanelOpen(false, true); }
async function seekFoliateProgress(percent) { const generation = ++navigationGeneration, sections = epubBook?.sections?.filter((section) => section.linear !== "no") || []; if (!sections.length || !foliateSectionLoader) return; const position = Math.max(0, Math.min(0.999999, Number(percent) / 100)) * sections.length, index = Math.min(sections.length - 1, Math.floor(position)), article = await foliateSectionLoader(index); if (foliateSectionSettler) await foliateSectionSettler(); if (!article || generation !== navigationGeneration) return; const marker = viewport.getBoundingClientRect().top + 8, offset = article.getBoundingClientRect().height * (position - index); viewport.scrollTop = Math.max(0, viewport.scrollTop + article.getBoundingClientRect().top - marker + offset); const section = sections[index], tocIndex = tocEntries.findIndex((entry) => { const target = epubBook.resolveHref(entry.href); return target && epubBook.sections[target.index] === section; }); if (tocIndex >= 0) currentChapterIndex = tocIndex; updateTocCurrentMark(); }
...
function clearFullSearchMarks() { for (const mark of fullSearchActiveMarks) { if (mark.tagName === "MARK") mark.replaceWith(mark.textContent); else mark.classList.remove("full-search-highlight"); } fullSearchActiveMarks = []; content.normalize(); }
...
async function start() {
  ...
  if ((!validSource(sourceUrl) && !validSource(chapterManifestUrl) && !readerId) || capability.readerMode === VoiceOfMLReader.ReaderMode.UNSUPPORTED) return fail("此文件暂不支持在线阅读，请下载原文件。");
  applyReaderMetadata(resolvedReaderData);
  ...
  try {
    if (capability.mode === "foliate") { readerStage = "foliate"; foliateContinuous = true; [restoredEntry] = await Promise.all([VoiceOfMLReaderStore.get(sourceUrl).catch(() => { restorationFailed = true; return null; }), renderFoliate(null)]); if (restoredEntry?.zoom) setZoom(restoredEntry.zoom, false); if (Number.isInteger(restoredEntry?.foliateSection)) await restoreFoliateBookmarkPosition(restoredEntry); else if (restoredEntry) viewport.scrollTop = restoredEntry.scrollTop || 0; restorationReady = !restorationFailed; scheduleSave(); return; }
    readerStage = "prepare";
...
function loadFoliateDocument() { return import("/search/static/foliate-reader/view.js?reader-v1").catch((error) => { readerStage = "foliate-import"; throw error; }); }
