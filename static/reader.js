...
async function renderPdfPages(prepared) {
  ...
  pdfShellsReady = (async () => { for (let start = 2; start <= documentState.pageCount; start += 24) { const fragment = document.createDocumentFragment(); for (let page = start; page < Math.min(start + 24, documentState.pageCount + 1); page++) fragment.appendChild(createShell(page)); content.appendChild(fragment); await new Promise((resolve) => setTimeout(resolve, 0)); } })();
