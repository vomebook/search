...
function registerReaderFormatAdapters() { for (const mode of Object.keys(VoiceOfMLReaderRuntime.FEATURE_MATRIX).filter((item) => item !== "unsupported")) formatAdapters.register(mode, { open: () => prepareDocument(), render: (prepared) => renderByMode(prepared), navigate: (value) => ["pdf", "pdf-pages", "docx"].includes(mode) ? goToPage(value) : Promise.resolve(), search: (query) => { fullSearchInput.value = query || ""; return runFullSearch(); }, progress: () => captureBookmarkSnapshot(), restore: (entry) => entry ? restoreProgressState(entry) : Promise.resolve(), dispose: () => {} }); }
