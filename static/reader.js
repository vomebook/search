...
const readerLifecycle = { phase: "startup", disposed: false, errorCode: "" };
...
function setReaderStage(stage) { readerStage = stage; setReaderPhase(stage); }
...
async function start() {
  ...
  try {
    if (capability.mode === "foliate") { setReaderStage("foliate"); foliateContinuous = true; [restoredEntry] = await Promise.all([VoiceOfMLReaderStore.get(sourceUrl).catch(() => { restorationFailed = true; return null; }), renderFoliate(null)]); if (restoredEntry?.zoom) setZoom(restoredEntry.zoom, false); if (Number.isInteger(restoredEntry?.foliateSection)) await restoreFoliateBookmarkPosition(restoredEntry); else if (restoredEntry) viewport.scrollTop = restoredEntry.scrollTop || 0; restorationReady = !restorationFailed; setReaderPhase("ready"); scheduleSave(); return; }
    ...
    loadingIndicator.remove(); loadingStatus.hidden = true; if (!pageCount && restoredEntry) viewport.scrollTop = restoredEntry.scrollTop || 0; restorationReady = !restorationFailed; setReaderPhase("ready"); scheduleSave();
  } catch (error) {
    console.error(error);
