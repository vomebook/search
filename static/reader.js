...
function setToc(entries) { tocEntries = (entries || []).map((entry) => ({ ...entry, href: String(entry.href || ""), label: cleanTocLabel(entry.label) })); currentChapterIndex = tocEntries.length ? 0 : -1; const tocTab = document.querySelector("#toc-tab"); tocTab.hidden = false; tocTab.textContent = tocEntries.length ? "目录" : "阅读"; const previous = document.querySelector("#toc-list"), list = previous.cloneNode(false); previous.replaceWith(list); for (const [index, entry] of tocEntries.entries()) { const row = document.createElement("div"); row.className = "panel-item toc-item"; row.style.setProperty("--toc-depth", String(entry.depth || 0)); const link = document.createElement("div"); link.className = "panel-item-main"; link.tabIndex = 0; link.setAttribute("role", "link"); link.textContent = entry.label; const activate = async () => { if (getSelection().toString()) return; const generation = ++navigationGeneration; currentChapterIndex = index; try { if (foliateContinuous) await activateFoliateTocEntry(entry, generation); else { await entry.activate(); if (generation === navigationGeneration) setReaderPanelOpen(false, true); } } catch (error) { if (generation === navigationGeneration) console.warn("Reader navigation failed", error); } }; link.addEventListener("click", activate); link.addEventListener("keydown", (event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); activate(); } }); row.appendChild(link); list.appendChild(row); } }
...
function flushEpubSeek() { if (epubSeekFrame) { cancelAnimationFrame(epubSeekFrame); epubSeekFrame = 0; epubSeekPromise = Promise.resolve(epubRendition.display(epubBook.locations.cfiFromPercentage(epubSeekTarget / 100))).catch(() => {}); } return epubSeekPromise; }
function seekProgress(value, preserve = false) { if (!preserve || !previousProgressState) previousProgressState = captureProgressState(); const percent = Number(value); if (mediaElement && mediaElement.duration) mediaElement.currentTime = mediaElement.duration * percent / 100; else if (foliateContinuous) viewport.scrollTop = Math.max(0, viewport.scrollHeight - viewport.clientHeight) * percent / 100; else if (htmlFrame && htmlFrame.contentWindow) htmlFrame.contentWindow.scrollTo(0, Math.max(0, htmlFrame.contentDocument.documentElement.scrollHeight - htmlFrame.contentWindow.innerHeight) * percent / 100); else viewport.scrollTop = Math.max(0, viewport.scrollHeight - viewport.clientHeight) * percent / 100; updateProgressTools(); scheduleSave(); }
async function activateChapter(index) { if (index < 0 || index >= tocEntries.length || index === currentChapterIndex) return; currentChapterIndex = index; try { await tocEntries[index].activate(); } finally { updateProgressTools(); } }
...
async function start() {
  ...
  try {
    if (capability.mode === "foliate") { readerStage = "foliate"; foliateContinuous = true; await renderFoliate(null); restorationReady = !restorationFailed; scheduleSave(); return; }
    readerStage = "prepare";
