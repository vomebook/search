...
function navigateFormat(mode, value) { if (["pdf", "pdf-pages", "docx"].includes(mode)) return goToPage(value); if (["foliate", "epub-chapters"].includes(mode)) return navigateTocEntry(Number(value)); const entry = tocEntries[Number(value)]; return entry?.activate ? Promise.resolve(entry.activate()) : Promise.resolve(); }
