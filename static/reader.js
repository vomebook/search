...
function syncTocTab() { const tab = document.querySelector("#toc-tab"); if (!tab) return; if (!navigationState.tocEntries.length) { tab.hidden = false; tab.textContent = "阅读"; } else if (navigationState.currentChapterIndex < 0) updateNavigationState({ currentChapterIndex: 0 }); }
syncTocTab();
