...
function syncReaderState() { readerRuntime.update("document", { title, zoom, page: currentPage, pageCount, restoredEntry, restorationReady }); readerRuntime.update("navigation", { tocEntries, currentChapterIndex }); Object.assign(readerRuntime.state.search, { results: typeof fullSearchResults === "undefined" ? [] : fullSearchResults, index: typeof fullSearchIndex === "undefined" ? -1 : fullSearchIndex }); Object.assign(readerRuntime.state.panel, { showingAllBookmarks, editingBookmark }); Object.assign(readerRuntime.state.formats.pdf, { document: pdfDocument, manifest: pdfPageManifest, activeRenders: pdfActiveRenders }); Object.assign(readerRuntime.state.formats.foliate, { rendition: epubRendition, book: epubBook, continuous: foliateContinuous, repository: foliateChapterRepository, virtualizer: foliateSectionVirtualizer }); Object.assign(readerRuntime.state.formats.html, { frame: htmlFrame }); Object.assign(readerRuntime.state.formats.media, { element: mediaElement }); }
...
function registerReaderFormatAdapters() { for (const mode of Object.keys(VoiceOfMLReaderRuntime.FEATURE_MATRIX).filter((item) => item !== "unsupported")) formatAdapters.register(mode, { open: () => prepareDocumentDirect(), render: (prepared) => renderByMode(prepared), navigate: (value) => ["pdf", "pdf-pages", "docx"].includes(mode) ? goToPage(value) : Promise.resolve(), search: (query) => { fullSearchInput.value = query || ""; return runFullSearch(); }, progress: () => captureBookmarkSnapshot(), restore: (entry) => entry ? restoreProgressState(entry) : Promise.resolve(), dispose: () => {} }); }
