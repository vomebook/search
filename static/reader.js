...
let capability = VoiceOfMLReader.capability(extension);
...
const readerLifecycle = { phase: "startup", stage: "startup", disposed: false, errorCode: "" };
function setReaderPhase(phase) { if (readerLifecycle.disposed && phase !== "disposed") return false; readerLifecycle.phase = phase; document.documentElement.dataset.readerPhase = phase; return true; }
function setReaderStage(stage) { if (!setReaderPhase(stage)) return false; readerLifecycle.stage = stage; return true; }
...
const foliateScrollAnchors = VoiceOfMLReaderScroll.createScrollAnchorManager({ viewport, candidates: foliateSectionCandidates });
...
function scheduleSave() { if (readerLifecycle.disposed) return; clearTimeout(saveTimer); saveTimer = setTimeout(saveProgress, 500); }
async function saveProgress() {
  ...
  const shell = pageCount
    ...
    foliatePosition = captureFoliateBookmarkPosition();
...
async function renderFoliate(prepared) {
  ...
  const createSection = async (index) => {
    ...
    for (const image of named("image")) { const value = image.getAttributeNS("http://www.w3.org/1999/xlink", "href") || image.getAttribute("xlink:href") || image.getAttribute("href"); if (value && !/^(?:data:|blob:|https?:|#)/i.test(value)) { const proxy = assetUrl(value); image.setAttributeNS("http://www.w3.org/1999/xlink", "xlink:href", proxy); image.setAttribute("href", proxy); } }
      ...
      sectionBody.innerHTML = (body?.innerHTML || doc.documentElement?.innerHTML || "")
        .replace(/<\/html:/gi, "</")
        .replace(/<html:/gi, "<");
      shadow.append(baseStyle, ...named("style").map((node) => node.cloneNode(true)), ...named("link").filter((node) => node.getAttribute("rel") === "stylesheet").map((node) => node.cloneNode(true)), sectionBody);
      return article;
  };
