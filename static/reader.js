...
function syncEpubTocLocation(location) { const href = location && location.start && String(location.start.href || "").split("#")[0]; if (!href) return; const index = navigationState.tocEntries.findIndex((entry) => sameEpubPath(String(entry.href || "").split("#")[0], href)); if (index >= 0) { updateNavigationState({ currentChapterIndex: index }); updateTocCurrentMark(); } }
...
async function renderPdfPages(prepared) {
  ...
  pdfShellsReady = (async () => { for (let start = 2; start <= pageCount; start += 250) { const fragment = document.createDocumentFragment(); for (let page = start; page < Math.min(start + 250, pageCount + 1); page++) fragment.appendChild(createShell(page)); content.appendChild(fragment); await new Promise((resolve) => setTimeout(resolve, 0)); } })();
...
async function runFullSearch() { const query = fullSearchInput.value.trim();
  ...
  else if (capability.mode === "epub") { const contents = epubRendition && typeof epubRendition.getContents === "function" ? epubRendition.getContents() : [];
  if (!contents.length) throw new Error("EPUB 尚未加载可搜索章节");
  for (const item of contents) { updateSearchState({ results: [...searchState.results, ...fullSearchDomMatches(item.document.body, "EPUB 当前章节")].slice(0, 100) });
  if (searchState.results.length >= 100) break;
  }
...
async function restoreFormat(mode, entry) { if (!entry) return; if (mode === "foliate" && Number.isInteger(entry.foliateSection)) return restoreFoliateBookmarkPosition(entry); if (entry.epubLocation && epubRendition) return epubRendition.display(entry.epubLocation); if (entry.page) { await goToPage(entry.page); const shell = content.querySelector(`.reader-page[data-page="${entry.page}"], .reader-docx-page[data-page="${entry.page}"]`); if (shell && Number.isFinite(entry.pageOffset)) viewport.scrollTop = shell.offsetTop + entry.pageOffset; return; } restoreProgressState(entry); }
