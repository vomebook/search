/* ═══════════════════════════════════════════════════════════
   jieba-wasm Loader
   Uses dynamic import from CDN to avoid module timing issues.
...
(function() {
  ...
  window._jiebaReady = new Promise(function(resolve, reject) {
    // Fallback timeout: if CDN is unreachable after 15s, give up and resolve with null
    ...
    import(JIEBA_JS).then(function(mod) {
      ...
      // init() auto-detects WASM URL relative to the JS module URL,
      // but to be safe we pass the URL explicitly
      ...
      clearTimeout(fallbackTimer);
      console.warn("[jieba] Failed to load:", err.message || err);
      window._jiebaCut = null;
      window._jiebaCutAll = null;
      window._jiebaCutForSearch = null;
      resolve(null);  // resolve anyway so app proceeds with fallback
    });
  });
})();