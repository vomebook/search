(function (root) {
  ...
  (function preloadPdfOpeningResources() {
    try {
      ...
      const manifestRequest = fetch(manifestUrl.href, { priority: "low", signal: controller.signal })
        .then(async (response) => {
          const bytes = await security.readBytes(response, security.LIMITS.manifestBytes);
          return new Response(bytes, { status: response.status, statusText: response.statusText, headers: response.headers });
        })
        .finally(() => clearTimeout(timer));
      preload = { manifestUrl: manifestUrl.href, manifest: manifestRequest, pageUrl: pageUrl.href, image: null, dispose };
  ...
  function capability(extension) {
    ...
    return Object.freeze({
      ...
      article: !!mode,
      features: Object.freeze({ ...(features[mode] || { toc: false, search: false, zoom: false, bookmarks: false, pagination: false, media: false }) }),
    });
  }
